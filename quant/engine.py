...
# 공유 시세 저장소 — QuantInSight·ArkInsight 가 매일 수집·갱신하는 일봉/투자자 CSV.
# 일봉/투자자는 여기서 먼저 읽고, 없을 때만 arcmarket(pykrx/yfinance)·네이버로 폴백한다.
# (2026-07-29 사장 지시: "주가 데이터는 이미 수집 중이니 그 경로를 써라".)
QIS_DATA_DIR = os.environ.get("QIS_DATA_DIR", os.environ.get(
    "ARCTRADE_MARKET_DATA_DIR", os.path.join(
        os.environ.get("ARCTRADE_PRIVATE_DATA_DIR", os.path.expanduser("~/.local/share/arctrade")),
        "market_data")))
...
class DBManager:
    def __init__(self):
        ...
        private_root = os.environ.get(
            "ARCTRADE_PRIVATE_DATA_DIR", os.path.expanduser("~/.local/share/arctrade"))
        self.dir_name = os.environ.get(
            "ARCTRADE_QUANT_CACHE_DIR", os.path.join(private_root, "quant_cache"))
...
class CrawlerUtil:
    @staticmethod
    def _shared_daily(code, years, stop_date):
        """공유 저장소(QIS/data/daily_{code}.csv) 일봉 → 레거시 규격. 없으면 None."""
        path = os.path.join(QIS_DATA_DIR, f"daily_{str(code).zfill(6)}.csv")
        ...
        cutoff = datetime.today() - timedelta(days=int(years * 365))
        out = out[out["Date"] >= cutoff]
        for c in ("Open", "High", "Low", "Close", "Volume"):
            out[c] = pd.to_numeric(out.get(c, 0), errors="coerce").fillna(0).astype("int64")
    ...
    def _arcmarket_daily(code, years, stop_date):
        """일봉 → 레거시 규격(Date/Open/High/Low/Close/Volume, 오름차순).
        공유 저장소 우선 → arcmarket(pykrx/yfinance). 둘 다 없으면 None(→ 네이버 폴백)."""
        via = CrawlerUtil._shared_daily(code, years, stop_date)
    ...
    @staticmethod
    def _shared_investor(code, years, stop_date):
        """공유 저장소(QIS/data/investor_{code}.csv: date,inst_net,foreign_net) → 레거시 규격. 없으면 None."""
        path = os.path.join(QIS_DATA_DIR, f"investor_{str(code).zfill(6)}.csv")
        if not os.path.exists(path):
            return None
        try:
            df = pd.read_csv(path)
        ...
        if df is None or df.empty or "inst_net" not in df.columns:
            return None
        out = df.rename(columns={"date": "Date", "inst_net": "Inst_Net_Volume",
        ...
        if out.empty:
            return None
        for c in ("Inst_Net_Volume", "Foreign_Net_Volume"):
            out[c] = pd.to_numeric(out.get(c, 0), errors="coerce").fillna(0)
        return out[["Date", "Inst_Net_Volume", "Foreign_Net_Volume"]].sort_values("Date").reset_index(drop=True)

    @staticmethod
    def fetch_investor_data(code, years=1, stop_date=None):
        via = CrawlerUtil._shared_investor(code, years, stop_date)
        if via is not None and not via.empty:
            return via
